...
class YesNoUI(nextcord.ui.View):
    ...
    async def Yes(self, button: nextcord.ui.Button, interaction: nextcord.Interaction):
        fatal_yes_logs = self.bot.get_channel(
            1168259678174728202
        )  # IMPORTANT: TO BE CHANGED TO 1061608889637949480 AFTER ALEXY INVITES THE BOT TO THE MAIN SERVER
        e = nextcord.Embed(
            title=f"GuildCore Error! | {interaction.user} - {interaction.user.id} | {interaction.guild.name} - {interaction.guild_id}",
            description=f"```py\n{self.error}```",
